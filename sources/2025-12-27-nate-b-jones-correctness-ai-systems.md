# The Art of Defining "Correct" - Why AI Systems Fail Without Clear Standards
...
Este artigo chegou no momento certo. Estava discutindo com Claude sobre como definir "correctness" para o sistema de knowledge base que estou construindo.

Conceitos-chave que quero lembrar:

1. **Correctness = claims permitidas + evidência necessária + penalidades por erro vs silêncio**

2. **Moving goalposts** - o problema de redefinir o que é "correto" sem perceber. Isso conecta com a discussão sobre Regime Exploratório vs Regime Operacional.

3. **Dois regimes de ground truth:**
   - Fonte estruturada (dados, documentos) → correctness = encontrou certo e não inventou
   - Julgamento humano → correctness = ajudou a pensar melhor

4. **A skill que importa:** ser específico sobre o que você quer. IA expõe vagueza que humanos toleram.

Conexão com meus projetos:
- Sistema OS: está no regime operacional, precisa de correctness mais rígida
- AI-brain: está no regime exploratório, correctness pode evoluir
- A distinção entre âncoras (fixas) e lanterna (descoberta) que discutimos